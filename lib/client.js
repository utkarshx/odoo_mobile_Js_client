(function($, undefined) {
  $.extend({
    ...
    /***
    The ERP Server Implementation
    **/
  	erpserver: {
  		//The Server URL where it is hosted.
  		server: {
  			url: null,
  			db: null,
  			session_id: '',
  		...
  		paths: {
	    	...
	    	databases: '/web/database/get_list',
	    	dataset_call_kw: '/web/dataset/call_kw'
  		},

  		//This is the first function that has to be called up for the setting up of the server
  		setup: function(params){
  			this.server.url = params.server;
  		},
  		
  		//Getting the list of databases from the server
  		getDatabases: function(cb){
  			$.jsonRPC.setup({
				  endPoint : this.server.url + this.paths.databases,
				  namespace : ''
			  });
  			$.jsonRPC.request('call',{
  				params: {'session_id':this.server.session_id,'context':this.server.context},
				success: cb,
				error: function(result){
					console.log('Error in Get Datbases'+result);
				}
  			});
  		},

  		//For the authentication of the Users
  		auth: function(cb){

       $.jsonRPC.setup({
          endPoint : this.server.url + this.paths.auth,
          namespace : ''
        });
        $.jsonRPC.request('call',{
          params: { 'session_id': '','db': 'solidaridad','login': 'admin','password': 'soya','context':this.server.context},
        success: function(result){
          $.erpserver.server.context = result.result.user_context;
          console.log($.erpserver.server.context);
          cb(result);
        },
        error: function(result){
          console.log('Error in Authentication'+result);
        }
        });
  		},
      /* RPC */

      create: function (cb,cnt) {
        //  console.dir(context);
       ...
       $.jsonRPC.setup({
          endPoint : this.server.url + this.paths.dataset_call_kw,
          namespace : ''
        });
